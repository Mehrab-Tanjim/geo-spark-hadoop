...
class PCAUtils {
	 ...
	 */
	static Random random = new SecureRandom(); //new Random(System.currentTimeMillis());// TODO always zero not a good seed changed it to current time 
	...
	*/
	...
	static Vector denseVectorTimesMatrix(Vector vector, Matrix matrix,
			int start, int end, Vector xm_mahout) {
		int nRows = matrix.numRows();
		int nCols = matrix.numCols();
		for (int c = 0; c < nCols; c++) {
			double dotres = 0;
			for (int r = start; r < end; r++)
				dotres += vector.getQuick(r) * matrix.getQuick(r-start, c);
			xm_mahout.set(c, dotres);
		}
		return xm_mahout;
	}
	...
	public static void outerProductWithIndices(org.apache.spark.mllib.linalg.Vector yi, Vector ym, double[] xi,
			Vector xm, double[][] resArray, int[] nonZeroIndices, int start, int end) {
		// 1. Sum(Yi' x (Xi-Xm))
		...
		{
			yRow=nonZeroIndices[i];
			if(yRow>=start&&yRow<end){
				yScale=yi.apply(yRow);
				for (int xCol = 0; xCol < xSize; xCol++) {
					double centeredValue = xi[xCol] - xm.getQuick(xCol);
					resArray[yRow-start][xCol] += centeredValue * yScale;
				}
			}
	...
	public static void outerProductArrayInput(double[] yi,
			double[] xi, double[][] resArray) {

		int ySize = yi.length;
		int xSize = xi.length;
		for (int yRow = 0; yRow < ySize; yRow++) {
			for (int xCol = 0; xCol < xSize; xCol++) {
				double centeredValue = xi[xCol] ;
				resArray[yRow][xCol] += centeredValue * yi[yRow];
			}
		}
	}
	...
	public static Matrix updateXtXAndYtx(Matrix realCentralYtx,
			Vector realCentralSumX, Vector ym, Vector xm, int nRows,int start, int end) {
		for (int yRow = start; yRow < end; yRow++) {
			double scale = ym.getQuick(yRow);
			for (int xCol = 0; xCol < realCentralSumX.size(); xCol++) {
				double centeredValue = realCentralSumX.getQuick(xCol) - nRows
						* xm.getQuick(xCol);
				double currValue = realCentralYtx.getQuick(yRow-start, xCol);
				currValue -= centeredValue * scale;
				realCentralYtx.setQuick(yRow-start, xCol, currValue);
			}
		}
		return realCentralYtx;
	}
	...
	*/
	...
	public static Matrix convertSparkToMahoutMatrix(org.apache.spark.mllib.linalg.Matrix sparkMatrix)
	{
		int rows=sparkMatrix.numRows();
		int cols=sparkMatrix.numCols();
		Matrix mahoutMatrix=new DenseMatrix(rows,cols);
		for(int j=0;j<cols; j++)
		{
			for(int i=0; i< rows; i++)
			{
				mahoutMatrix.setQuick(i, j, sparkMatrix.apply(i, j));
			}
		}		
		return mahoutMatrix;
	}
	...
	public static void printMatrixInDenseTextFormat(Matrix m, String outputPath) {
		try {
			FileWriter fileWriter = new FileWriter(outputPath);
			PrintWriter printWriter= new PrintWriter(fileWriter);
			 for(int i=0; i < m.numRows(); i++)
			 {
				for(int j=0; j < m.numCols(); j++)
				{
					printWriter.print(m.getQuick(i, j) + " ");
				}
				printWriter.println();
			}
			printWriter.close();
	...
	public static void loadMatrixInDenseTextFormat(Matrix m, String outputPath) {
		try {
			File file=new File(outputPath);
			 ...
			 int row=0;
	          while ((thisLine = br.readLine()) != null) { // while loop begins here   		   
	        	  ...
	        	  for(int j=0; j < m.numCols(); j++)
					{
						m.setQuick(row, j, Double.parseDouble(splitted[j]));
					}
	        	  row++;
	          }
			 br.close();
	...
	public static void printStatToFile(Stat stat, String outputPath) {
		try {
			
			String fileLocation = outputPath+ File.separator +"stat.txt"; 
			File f = new File(fileLocation);
			
			String appname="App Name: "+stat.appName;
			String datasetname="Dataset Name: "+stat.dataSet;
			...
			for(int i=0;i<stat.errorList.size()-1;i++){
				error+=stat.errorList.get(i)+", ";
			}
			if(!stat.errorList.isEmpty())
				error+=stat.errorList.get(stat.errorList.size()-1)+"]";
